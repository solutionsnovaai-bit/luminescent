...
// o frete escolhido pelo cliente + os dados de entrega, RECALCULA tudo a
// partir do servidor — nunca confia em preço vindo do navegador, nem no
// de produto nem no de frete — e cria uma preferência de pagamento no
// Mercado Pago (Checkout Pro). Devolve `init_point`, a URL pra onde o
// front-end redireciona o cliente.
//
...
//
// Dados de entrega: nome, telefone e endereço são coletados no próprio
// carrinho do site (não na tela do Mercado Pago) e vão dentro de
// `metadata` — aparecem nos detalhes do pagamento, no painel da própria
// Naara, sem precisar de banco de dados.
...
/** Corta um campo de texto num tamanho razoável, sem travar em string vazia/ausente. */
...
export default async function handler(req, res) {
  ...
  // ── Dados de entrega — obrigatórios pra Naara saber pra quem e pra
  //    onde enviar. Revalidados aqui também (nunca só no navegador). ──
  const customerName = clean(body?.customerName);
  const customerPhone = clean(body?.customerPhone).replace(/\D/g, '');
  ...
  if (customerName.length < 3 || customerPhone.length < 10 || !street || !streetNumber || !neighborhood) {
    return res.status(400).json({ error: 'Preencha seus dados de entrega antes de continuar.' });
  }
  ...
  const preference = {
    ...
    // Mercado Pago (Detalhes → Metadata) — incluindo pra quem e pra onde
    // enviar, já que isso nunca é perguntado na tela de pagamento deles.
    metadata: {
