...
export default function ProductCard({ p, onAdd, onOpen }) {
  ...
  // troca sozinha entre as duas fotos a cada alguns segundos — só pros
  // produtos que têm uma segunda foto (image2). Reinicia a contagem
  // sempre que a pessoa mexe manualmente (clique ou arrasto), pra nunca
  // "brigar" com um toggle que acabou de acontecer.
  const restartTimer = useCallback(() => {
    if (timerRef.current) clearInterval(timerRef.current);
    if (!hasAlt || reducedRef.current) return;
    timerRef.current = setInterval(() => setShowAlt((v) => !v), AUTO_MS);
  }, [hasAlt]);
  ...
  const onTouchEnd = (e) => {
    ...
    if (Math.abs(dx) > SWIPE_THRESHOLD) toggleAlt();
    touchX.current = null;
  };
    ...
    <article className="reveal group flex flex-col">
      <div className="relative mb-5 overflow-hidden rounded-sm bg-white/60">
        ...
        {hasAlt && (
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); toggleAlt(); }}
